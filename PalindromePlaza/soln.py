...
def gen_cases(case_count,base_dir):
    ...
    for i in range(case_count):
        ...
        file_name = base_dir + "/input/input{}{}.txt".format(i//10,i%10)
        with open(file_name,"w") as f:
            print(word_size, file=f)
...
def gen_solns(base_dir):
    # base dir == samples
    # read the files
    for f in os.listdir(base_dir + "/input"):
        file_name = os.path.join(base_dir+"/input",f)
        # read file
        soln = solve_from_file(file_name)
        # write to file
        number_of_file = re.findall(r'\d+',file_name)[0]
        output_name = base_dir + "/output/output{}.txt".format(number_of_file)
        with open(output_name,"w") as f:
            print(soln,file=f)

## gen cases and then solns
gen_cases(50,"./samples")
gen_solns("./samples")
